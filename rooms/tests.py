...
from rooms.models import Amenity, Booking, Profile, Room
...
class CustomUserModelTests(TestCase):
    ...
    def test_create_superuser(self):
        ...
        admin_user = User.objects.create_superuser(
            email="super@user.com", username="testsuperuser", password="foo"
        )
...
class ProfileModelTests(TestCase):
    ...
    def test_profile_creation(self):
        profile = Profile.objects.create(
            user=self.user, phone_number="1234567890", address="123 Test St"
        )
...
class ViewTests(TestCase):
    ...
    def test_book_room_view_unauthenticated(self):
        ...
        self.assertRedirects(
            response, f"{login_url}?next=/book-room/{self.room.id}/"
        )
...
class BookingManagementTests(TestCase):
    ...
    def test_edit_booking(self):
        ...
        self.assertEqual(
            self.booking.check_in_date.strftime("%Y-%m-%d"), new_check_in
        )
        self.assertEqual(
            self.booking.check_out_date.strftime("%Y-%m-%d"), new_check_out
        )
    ...
    def test_cancel_booking(self):
        ...
        # Cancel booking
        response = self.client.post(
            reverse("cancel_booking", args=[self.booking.id]), follow=True
        )

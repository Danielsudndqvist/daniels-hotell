...
class Profile(models.Model):    
    user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)  # Link to the CustomUser model
...
class Room(models.Model):
    ...
    room_type = models.CharField(max_length=3, choices=ROOM_TYPES)  # Room type selection
    ...
    amenities = models.ManyToManyField(Amenity, blank=True)  # Relationship to amenities (many-to-many)
    max_occupancy = models.IntegerField(default=2)  # Maximum number of guests
    size = models.IntegerField(help_text="Size in square feet", default=0)  # Size of the room in square feet
...
class RoomImage(models.Model):
    room = models.ForeignKey(Room, related_name='images', on_delete=models.CASCADE)  # Link to the Room model
    image = models.ImageField(upload_to='room_images/')  # Image field for room images
    caption = models.CharField(max_length=100, blank=True)  # Optional caption for the image
...
class Booking(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null=True, blank=True)  # Link to the user making the booking
    room = models.ForeignKey(Room, on_delete=models.CASCADE)  # Link to the room being booked
    ...
    def __str__(self):
        # Return a description of the booking including the room name and guest name
